...
public class Ji extends MapObject{

	private int health;
	private int maxHealth;
	private TileMap tiley;
	
	private double bounceSpeed = -10.2; //how high other person bounces
	...
	public Ji(TileMap tm, boolean righting){
		super(tm);
		this.tiley = tm;
		...
		try {
			...
			BufferedImage spritesheet = ImageIO.read(
					getClass().getResourceAsStream("/Sprites/GreenSheet.png"));
		...
		sfx.put("jump", new AudioPlayer("/Audio/FX/Jumping/JiJump.mp3"));
	...
	public boolean checkCatch(Zav zav){
		
		boolean caught = false;
		
		if(zav.getx() - 10 < getx() && (zav.getx() + zav.getCWidth() + 10) > (getx() + getCWidth())){ 
			if(zav.gety() + zav.getCHeight() <= gety() &&
					zav.gety() + zav.getCHeight() >= gety() - 5){
				if(zav.getCurrentAction() == FALLING){
						
					sfx.get("bounce").play();
					zav.setDy(bounceSpeed);
				}
